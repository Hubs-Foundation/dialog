...
class Room extends EventEmitter
{
	...
	// static getLeastLoadedRouterId(mediasoupWorkers, peers, mediasoupRouters)
	// {
	...
	// 	{
	// 		const routerId = peer.data.routerId;
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		var workerIdx = await utils.getLeastLoadedWorkerIdx(mediasoupWorkers)
		var worker = mediasoupWorkers[workerIdx]
		logger.info("least loaded worker._pid: ", worker._pid)
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			index.workerLoadMap.set(peer.data.workerPid, index.workerLoadMap.get(peer.data.workerPid) - 1)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				index.workerLoadMap.set(peer.data.workerPid, (index.workerLoadMap.get(peer.data.workerPid) ?? 1) + 1)
			...
			case 'produce':
			{
				...
				// const otherRouters = this._getRoutersToPipeTo(peer.data.routerId);
				...
				{
					if (otherRouters.includes(routerId))
					{
						logger.info(
							" produce -> producer.id: ", producer.id, 
							", piping from routerId: ",srcRouter.id," to routerId: ",  router.id)
	...
	async _getRouterId()
	{
		...
		var leastusedWorkerIdx = await utils.getLeastLoadedWorkerIdx(workers)
		const worker = workers[leastusedWorkerIdx]
		const routerId = this._inuseMediasoupWorkers.get(worker)
		logger.info("_getRouterId: ", routerId)
	...
	// 				continue;
	// 			}
	// 			logger.warn(" ---_pipeProducersToRouter-(producerId)", producerId, "--- piping from (srcRouterId)",srcRouter.id," to (routerId)",  router.id)
	...
	// _getotherRouters(originRouterId)
	// {
	// 	const routers = Object.values(this._inuseMediasoupWorkers).filter((routerId, self)=>routerId!==originRouterId)
	// 	logger.warn("_getRoutersToPipeTo --> routers: ",routers)
	// 	return routers
	// }
