...
class Room extends EventEmitter
{
	...
	static async getLeastLoadedWorker(mediasoupWorkers){
		const minUsage=Number.MAX_VALUE
		let leastLoadedWorker
		for (const worker of mediasoupWorkers)
		{			
			const usage = await worker.getResourceUsage().ru_utime
			if (usage < minUsage) {
				minUsage = usage
				logger.warn("new minUsage:", minUsage)
				leastLoadedWorker = worker 
			}
			logger.warn("pid:", worker._pid, ", utime:", usage)
		}
		logger.warn("global minUsage:", minUsage)
		logger.warn("leastLoadedWorker._pid:",leastLoadedWorker._pid)
