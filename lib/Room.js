...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		logger.warn(" using hardcoded (25) for roomReq -- TODO: request room with roomSize for different tiers...ie. request the lesser of (roomSize, workerPeerCapacity)")
		utils.workerLoadMap_addRoomReq(worker._pid, roomId, 25)	//TODO: 
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			utils.workerLoadMap_addPeer(peer.data.workerPid, -1)
			...
			if (this._protooRoom.peers.length === 0)
			{
				...
				for (worker of this._inuseMediasoupWorkers.keys()){
					utils.workerLoadMap_removeRoom(worker._pid, this._roomId)
				}	
	...
	async _getRouterId()
	{
		...
		if (peerCnt < utils.workerPeerCapacity){
		...
		//inuse workers are all maxed out
			...
			//try to find an available worker among unused workers
			...
			if(peerCnt_unused < utils.workerPeerCapacity){
