...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		var workerIdx = await this.getLeastLoadedWorkerIdx(unusedMediasoupWorkers)
		logger.warn("workerIdx: ", workerIdx)

		var worker = unusedMediasoupWorkers[workerIdx]
	 ...
	 */
	static async _getRouterId()
	{
		const workers = Array.from( this._inuseMediasoupWorkers.keys() );
		var leastusedWorkerIdx = await this.getLeastLoadedWorkerIdx(workers)
