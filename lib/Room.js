...
class Room extends EventEmitter
{
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			workerLoadMap.set(peer.data.workerPid, workerLoadMap.get(peer.data.workerPid) - 1)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				workerLoadMap.set(peer.data.workerPid, (workerLoadMap.get(peer.data.workerPid) ?? 1) + 1)
