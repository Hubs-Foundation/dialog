...
class Room extends EventEmitter
{
	...
	static async getLeastLoadedWorker(mediasoupWorkers){
		const worker_ustime = new Map();
		for (const worker of mediasoupWorkers)
		{
			const usage = await worker.getResourceUsage()
			const stime = usage.ru_utime
			worker_ustime.set(worker._pid, stime)
		}
		const sortedWorkersPids = new Map([ ...worker_ustime.entries() ].sort(
			(a, b) => a[1] - b[1]));
		logger.warn("sortedWorkersPids", sortedWorkersPids)
		const LeastUsedWorkerPid = sortedWorkersPids.keys().next().value;
		logger.warn("LeastUsedWorkerPid", LeastUsedWorkerPid)
		return sortedWorkers.get(LeastUsedWorkerPid)
	}
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		const worker = this.getLeastLoadedWorker(mediasoupWorkers)
		mediasoupWorkers.splice(mediasoupWorkers.indexof(worker), 1)
