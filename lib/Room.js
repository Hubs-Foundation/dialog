...
class Room extends EventEmitter
{
	...
	static async getLeastLoadedWorker(mediasoupWorkers){
		...
		for (const worker of mediasoupWorkers)
		{
			...
			worker_ustime.set(worker, stime)
		}
		const sortedWorkers = new Map([ ...worker_ustime.entries() ].sort(
			(a, b) => a[1] - b[1]));
		logger.warn("sortedWorkers", sortedWorkers)
		const LeastUsedWorker = sortedWorkers.keys().next().value;
		logger.warn("LeastUsedWorkerPid", LeastUsedWorker)

		return worker_ustime.get(LeastUsedWorker)
	}
