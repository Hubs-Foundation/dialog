...
class Room extends EventEmitter
{
	...
	static async getLeastLoadedWorker(mediasoupWorkers){
		const workers = new Map()
		const workers_ustime = new Map()
		for (const worker of mediasoupWorkers)
		{
			const usage = await worker.getResourceUsage()
			const stime = usage.ru_utime
			workers_ustime.set(worker, stime)
		}
		const sortedWorkers = new Map([ ...workers_ustime.entries() ].sort(
			(a, b) => a[1] - b[1]));
		logger.warn("sortedWorkers.values", sortedWorkers.values())
		const LeastUsedWorker = sortedWorkers.keys().next().value;
		logger.warn("LeastUsedWorker", sortedWorkers.get(LeastUsedWorker))
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		const worker = this.getLeastLoadedWorker(unusedMediasoupWorkers)
	...
	async _getRouterId()
	{
		...
		const worker = this.getLeastLoadedWorker(workers)
