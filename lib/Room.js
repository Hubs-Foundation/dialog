...
class Room extends EventEmitter
{
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			utils.workerLoadMap.set(peer.data.workerPid, utils.workerLoadMap.get(peer.data.workerPid) - 1)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				utils.workerLoadMap.set(peer.data.workerPid, (utils.workerLoadMap.get(peer.data.workerPid) ?? 1) + 1)
