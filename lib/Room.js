...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		for (const worker of mediasoupWorkers)
		{
			...
			logger.info("created -- rouer.id:", router.id)
			mediasoupRouters.set(router.id, router);
		}

		r = Room.getLeastLoadedRouterId(mediasoupWorkers, protooRoom.peers, mediasoupRouters)
		const firstRouter = mediasoupRouters.get(r);
		logger.info("firstRouterId: ", r)
