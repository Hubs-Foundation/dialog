...
class Room extends EventEmitter
{
	 ...
	 */
	static async create({ mediasoupWorkers, roomId, authKey, room_size=0 })
	{
		...
		var [workerIdx, peerCnt] = utils.workerLoadMan.getLeastLoadedWorkerIdx(mediasoupWorkers)
		...
		var consumerBudget=this._calculateConsumerBudget(room_size)
		logger.info(" consumerBudget %s", consumerBudget)
		if (consumerBudget==0){
			logger.warn("roomId <%s> didn't reserve any capacity (consumerBudget==%s) -- room overload possible", roomId, consumerBudget)
		}
		// utils.workerLoadMan.addRoomReq(worker._pid, roomId, consumerBudget)
	...
	constructor({ roomId, consumerBudget,protooRoom, mediasoupRouters, audioLevelObserver, arr_unusedMediasoupWorkers, inuseMediasoupWorkers, bot, authKey })
	{
		super();
		...
		this._consumerBudget = consumerBudget
	...
	getPeers(){
		if (!this._protooRoom || !this._protooRoom.peers) return null;
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			// utils.workerLoadMan.addPeer(peer.data.workerPid, -1, peer.id)
			...
			if (this._protooRoom.peers.length === 0)
			{
				...
				this.close();
				// for (worker of this._inuseMediasoupWorkers.keys()){
				// 	utils.workerLoadMan.removeRoom(worker._pid, this._roomId)
				// }	
	...
	async _getRouterId()
	{
		...
		var [leastusedWorkerIdx, peerCnt] = utils.workerLoadMan.getLeastLoadedWorkerIdx(workers)

		logger.info("_inuseMediasoupWorkers -> leastusedWorkerIdx: %s, peerCnt %s", leastusedWorkerIdx, peerCnt)

		if (peerCnt < utils.maxPerCoreCCU){
		...
		//inuse workers are all maxed out
			...
			var [leastusedWorkerIdx_unused, peerCnt_unused] = utils.workerLoadMan.getLeastLoadedWorkerIdx(this._arr_unusedMediasoupWorkers)
			logger.info("leastusedWorkerIdx_unused: %s, peerCnt: %s", leastusedWorkerIdx_unused, peerCnt_unused)
			if(peerCnt_unused < utils.maxPerCoreCCU){
	...
	static _calculateConsumerBudget(room_size){
		var producerBudget = Math.floor(room_size/5) +1
		return room_size*producerBudget
	}
