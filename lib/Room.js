...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		var workerIdx = utils.getLeastLoadedWorkerIdx(mediasoupWorkers)
		var worker = mediasoupWorkers[workerIdx]
		logger.info("workerIdx %s, worker %s", workerIdx, worker)
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('close', () =>
		{
			...
			utils.workerLoadMap_set(
				peer.data.workerPid, 
				utils.workerLoadMap_get(peer.data.workerPid) - 1
				)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				utils.workerLoadMap_set(
					peer.data.workerPid, 
					(utils.workerLoadMap_get(peer.data.workerPid) ?? 1) + 1
					)
