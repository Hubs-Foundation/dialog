	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'disableConsumer':
			{
				...
				if (!producer) {
					logger.warn(`producer from peer "${producerPeer.id}" of type "${kind}" not found`);
					accept();
					return;
				}
				...
				if (!consumer) {
					logger.warn(`consumer for peer "${otherPeerId}" of type "${kind}" not found`);
					accept();
					return;
				}
			...
			case 'enableConsumer':
			{
				...
				const producerPeer = this._getJoinedPeers({ excludePeer: peer }).filter((joinedPeer) => joinedPeer.id === otherPeerId).pop()
				if (!producerPeer) {
					logger.warn(`producer with id "${otherPeerId}" not found`);
					accept();
					return;
				}
				...
				if (consumer) {
					logger.warn(`consumer for peer "${otherPeerId}" of type "${kind}" already exists`);
					accept();
					return;
				}
