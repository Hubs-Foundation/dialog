...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey, room_size=0 })
	{
		 ...
		 */		
		const roomReq=Math.floor(Math.sqrt(room_size))
		...
		const [workerIdx, peerCnt] = utils.workerLoadMan.getLeastLoadedWorkerIdx(mediasoupWorkers, roomId, this._roomReq)
	...
	async _getRouterId()
	{
		...
		//inuse workers are all maxed out
			...
			const [leastusedWorkerIdx_unused, peerCnt_unused] = utils.workerLoadMan.getLeastLoadedWorkerIdx(this._arr_unusedMediasoupWorkers, this._roomId, this._roomReq)
