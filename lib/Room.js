...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		// Create mediasoup Routers on all workers
		...
		const bot = await Bot.create({ mediasoupRouter: firstRouter });
	...
	constructor({ roomId, protooRoom, mediasoupRouters, audioLevelObserver, mediasoupWorkers, bot, authKey })
	{
		super();
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		logger.info("peer.data.routerId", peer.data.routerId)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		logger.info("peer.data.routerId: ", peer.data.routerId)			
		logger.info("this._mediasoupRouters: ", this._mediasoupRouters)
	...
	async _getRouterId()
	{
		const routerId = Room.getLeastLoadedRouterId(
			this._mediasoupWorkers, this._protooRoom.peers, this._mediasoupRouters);
		
		logger.info("getLeastLoadedRouterId: ", routerId)
