...
class Room extends EventEmitter
{
	 ...
	 */
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		var [workerIdx, peerCnt] = utils.getLeastLoadedWorkerIdx(mediasoupWorkers)
		var worker = mediasoupWorkers[workerIdx]
		const router = await worker.createRouter({ mediaCodecs });
		mediasoupRouters.set(router.id, router);
		inuseMediasoupWorkers.set(worker, router.id)

		const unusedMediasoupWorkers = mediasoupWorkers.slice();
		unusedMediasoupWorkers.splice(unusedMediasoupWorkers.indexOf(worker), 1)

		logger.info("unusedWorkers: ", unusedMediasoupWorkers.length, ", inuseWorkers: ", inuseMediasoupWorkers.size)
		...
		// const firstRouterId = Room.getLeastLoadedRouterId(mediasoupWorkers, protooRoom.peers, mediasoupRouters)
		// const firstRouter = mediasoupRouters.get(firstRouterId);
		...
		const bot = await Bot.create({ mediasoupRouter: router });
	...
	constructor({ roomId, protooRoom, mediasoupRouters, audioLevelObserver, unusedMediasoupWorkers, inuseMediasoupWorkers, bot, authKey })
	{
		super();
		...
		// unused mediasoupWorkers for this room
		this._unusedMediasoupWorkers = unusedMediasoupWorkers;
		...
		this._mediasoupRouter = mediasoupRouters.entries().next().value;
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		var [routerId,workerPid]  = await this._getRouterId();
		...
		peer.on('close', () =>
		{
			...
			utils.workerLoadMap_add(peer.data.workerPid, -1)
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'produce':
			{
				...
				var otherRouters = Object.keys(this._mediasoupRouters).filter((id)=>id!==peer.data.routerId)
				logger.info(" produce -> otherRouters: ", otherRouters)

				for (const [ routerId, targetRouter ] of otherRouters)
				{
					logger.info( "piping from (routerId)%s to (rouerId)%s for (producerId) %s",srcRouter.id, targetRouter.id, producer.id)
	...
	async _getRouterId()
	{
		...
		var [leastusedWorkerIdx, peerCnt] = utils.getLeastLoadedWorkerIdx(workers)

		logger.info("this._inuseMediasoupWorkers", this._inuseMediasoupWorkers)

		if (peerCnt < peersPerRouter){
			const worker = workers[leastusedWorkerIdx]
			const routerId = this._inuseMediasoupWorkers.get(worker)
			return [routerId, worker._pid]
		}
		//inuse workers are all maxed out

		logger.info("this._unusedMediasoupWorkers", this._unusedMediasoupWorkers)
		logger.info("this._unusedMediasoupWorkers.size", this._unusedMediasoupWorkers.size)

		if (this._unusedMediasoupWorkers.size > 0){
			//try to find an available worker among the unused
			const unusedWorkers = Array.from( this._unusedMediasoupWorkers.keys() );
			var [leastusedWorkerIdx_unused, peerCnt_unused] = utils.getLeastLoadedWorkerIdx(unusedWorkers)
			logger.info("peerCnt_unused",peerCnt_unused)
			if(peerCnt_unused < peersPerRouter){
				const worker = workers[leastusedWorkerIdx_unused]
				const { mediaCodecs } = config.mediasoup.routerOptions;
				...
				this._unusedMediasoupWorkers.splice(this._unusedMediasoupWorkers.indexOf(worker), 1)
				logger.info("new router (id: %s) created on worker (pid: %s)", newRouter.id, worker._pid)
