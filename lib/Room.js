...
class Room extends EventEmitter
{
	...
	async handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.data.disabledConsumers = new Map();
		...
		peer.on('close', () =>
		{
			...
			if (peer.data.joined)
			{
				...
				{
					otherPeer.data.disabledConsumers.delete(peer.id);
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'produce':
			{
				...
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					const disabledConsumers = otherPeer.data.disabledConsumers.get(peer.id);
					if (!disabledConsumers || !disabledConsumers.has(kind)) {
			...
			case 'closeProducer':
			{
				...
				logger.debug(`Producer for peer ${peer.id} closed`);
			...
			case 'pauseProducer':
			{
				...
				logger.debug(`Producer for peer ${peer.id} paused`);
			...
			case 'resumeProducer':
			{
				...
				logger.debug(`Producer for peer ${peer.id} resumed`);
			...
			case 'disableConsumer':
			{
				// Ensure the Peer is joined.
				if (!peer.data.joined)
					throw new Error('Peer not yet joined');

				const { otherPeerId, kind } = request.data;

				const producerPeer = this._getJoinedPeers({ excludePeer: peer }).filter((joinedPeer) => joinedPeer.id === otherPeerId).pop()
				if (!producerPeer)
					throw new Error(`producer with id "${otherPeerId}" not found`);

				const producer = [...producerPeer.data.producers.values()].filter(producer => producer.kind === kind).pop();
				if (!producer)
					throw new Error(`producer from peer "${producerPeer.id}" of type "${kind}" not found`);
					
				const consumer = [...peer.data.consumers.values()].filter(consumer => consumer.producerId === producer.id && consumer.kind === kind).pop();
				if (!consumer)
					throw new Error(`consumer for peer "${otherPeerId}" of type "${kind}" not found`);

				await consumer.close();

				const disabledConsumers = peer.data.disabledConsumers.get(producerPeer.id) || new Set();
				if (!disabledConsumers.has(kind)) {
					disabledConsumers.add(kind);
				}
				peer.data.disabledConsumers.set(producerPeer.id, disabledConsumers);

				peer.notify('consumerClosed', { consumerId: consumer.id })
					.catch(() => {});

				logger.debug(`Consumer for peer ${otherPeerId} of kind ${kind} closed`);
			...
			case 'enableConsumer':
			{
				...
				const consumer = [...peer.data.consumers.values()].filter(consumer => consumer.producerId === producerPeer.id && consumer.kind === kind).pop();
				if (consumer)
					throw new Error(`consumer for peer "${otherPeerId}" of type "${kind}" already exists`);
				...
				const disabledConsumers = peer.data.disabledConsumers.get(producerPeer.id);
					if (disabledConsumers) {
						disabledConsumers.delete(kind);
					}

				logger.debug(`Consumer for peer ${otherPeerId} of kind ${kind} enabled`);
			...
			case 'pauseConsumer':
			{
				...
				logger.debug(`Consumer for peer ${peer.id} paused`);
			...
			case 'resumeConsumer':
			{
				...
				logger.debug(`Consumer for peer ${peer.id} resumed`);
