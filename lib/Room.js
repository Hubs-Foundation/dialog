	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'createConsumer':
			{
				...
				const { producerPeerId, producerId } = request.data;

				const producerPeer = this._getJoinedPeers({ excludePeer: peer }).filter((joinedPeer) => joinedPeer.id === producerPeerId).pop()
				if (!producerPeer)
					throw new Error(`producer with id "${producerPeerId}" not found`);

				const producer = producerPeer.data.producers.get(producerId);
