...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey })
	{
		...
		logger.info("unusedWorkers: ", arr_unusedMediasoupWorkers.length, ", inuseWorkers: ", inuseMediasoupWorkers.size)
	...
	constructor({ roomId, protooRoom, mediasoupRouters, audioLevelObserver, arr_unusedMediasoupWorkers, inuseMediasoupWorkers, bot, authKey })
	{
		super();
	...
	async _getRouterId()
	{
		...
		logger.info("_inuseMediasoupWorkers -> leastusedWorkerIdx: ", leastusedWorkerIdx)
		if (peerCnt < peersPerRouter){
			var worker = workers[leastusedWorkerIdx]
			const routerId = this._inuseMediasoupWorkers.get(worker)
			return [routerId, worker._pid]
		}
		//inuse workers are all maxed out
		if (this._arr_unusedMediasoupWorkers.length > 0){
			//try to find an available worker among the unused
			var [leastusedWorkerIdx_unused, peerCnt_unused] = utils.getLeastLoadedWorkerIdx(this._arr_unusedMediasoupWorkers)
			logger.info("leastusedWorkerIdx_unused: ", leastusedWorkerIdx_unused)
			if(peerCnt_unused < peersPerRouter){
				var worker_unused = workers[leastusedWorkerIdx_unused]
				const { mediaCodecs } = config.mediasoup.routerOptions;
				const newRouter = await worker_unused.createRouter({ mediaCodecs });
				this._mediasoupRouters.set(newRouter.id, newRouter)
				this._inuseMediasoupWorkers.set(worker_unused, newRouter.id)
				this._arr_unusedMediasoupWorkers.splice(this._arr_unusedMediasoupWorkers.indexOf(worker_unused), 1)
				logger.info("new router (id: %s) created on worker (pid: %s)", newRouter.id, worker_unused._pid)
				
				return [newRouter.id, worker_unused._pid]
			}
		}
