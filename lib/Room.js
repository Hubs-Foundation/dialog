...
class Room extends EventEmitter
{
	...
	static async create({ mediasoupWorkers, roomId, authKey, room_size=0 })
	{
		 ...
		 */		
		var roomReq=Math.floor(Math.sqrt(room_size))
		...
		var [workerIdx, peerCnt] = utils.workerLoadMan.getLeastLoadedWorkerIdx(mediasoupWorkers, this._roomId, this._roomReq)
	...
	async _getRouterId()
	{
		...
		var [leastusedWorkerIdx, peerCnt] = utils.workerLoadMan.getLeastLoadedWorkerIdx(workers, this._roomId, this._roomReq)
		...
		//inuse workers are all maxed out
			...
			var [leastusedWorkerIdx_unused, peerCnt_unused] = utils.workerLoadMan.getLeastLoadedWorkerIdx(this._arr_unusedMediasoupWorkers, this._roomId, this._roomReq)
