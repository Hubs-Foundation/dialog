...
class workerLoadMan{
    ...
    static addRoomReq(k, roomId, amt=9999999){
        ...
        //roomReqCnt
        if(!_workerLoadMap.has(k)){_workerLoadMap.set(k, {peerCnt:0, roomReqCnt:0, rooms:new Map()})}
        var sel = _workerLoadMap.get(k)
        sel.roomReqCnt += amt
    ...
    static surveyRooms(){
        ...
        for (var [id, room] of rooms.entries()){       
            logger.info("surveyRooms -- id: %s", id)

            for (var peer of room.getPeers()){
                this.addPeer(peer.data.workerPid, 1)
            }
            for (var worker of room._inuseMediasoupWorkers){
    ...
    static resetWorkerLoadMap(){
        for (var [k, v] of _workerLoadMap.entries()){
            logger.info("resetWorkerLoadMap-- k: %s", k)
            v.peerCnt=0
            v.roomReqCnt=0
            v.rooms=new Map()
        }
