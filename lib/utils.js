...
class utils{
     ...
     */
     static workerLoadMap_addRoomReq(k, roomId, amt=1){
        //rooms
        // workerLoadMap.get(k).rooms.has(roomId) ? 
        //     workerLoadMap.get(k).rooms.set(roomId, workerLoadMap.get(k).rooms.get(roomId) + amt) : 
        //         workerLoadMap.get(k).rooms.set(roomId, amt)
        if (!workerLoadMap.get(k).rooms.has(roomId)){
            logger.info("!workerLoadMap.get(k).rooms.has(roomId) --- setting %v, %v", roomId, amt)
            workerLoadMap.get(k).rooms.set(roomId, amt)
        }else{
            var newAmt = workerLoadMap.get(k).rooms.get(roomId) + amt
            logger.info("workerLoadMap.get(k).rooms.has(roomId) --- setting %v, %v", roomId, workerLoadMap.get(k).rooms.get(roomId) + newAmt)
            workerLoadMap.get(k).rooms.set(roomId, newAmt)
        }
        //roomReqCnt
        var current = workerLoadMap.get(k)
    ...
    static workerLoadMap_removeRoom(k, roomId){
        ...
        var current = workerLoadMap.get(k)
        workerLoadMap.set(k, {peerCnt: current.peerCnt, rooms: current.rooms,
            roomReqCnt:current.roomReqCnt - current.rooms.get(roomId)})

        //remove roomId in rooms
        workerLoadMap.get(k).rooms.delete(roomId)
    }
