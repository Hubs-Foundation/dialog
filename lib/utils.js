...
class workerLoadMan{
    ...
    static addPeer(k, amt=1, peerId=null, roomId="???"){
        ...
        _workerLoadMap.set(k, {roomReqCnt:sel.roomReqCnt, rooms: sel.rooms, consumerCnt: sel.consumerCnt,
            peerCnt: sel.peerCnt+amt})
    }

    static addConsumer(k, amt=1){
        var sel = _workerLoadMap.get(k)
        _workerLoadMap.set(k, {roomReqCnt:sel.roomReqCnt, rooms: sel.rooms, peerCnt: sel.peerCnt,
    ...
    static runSurvey(){
        ...
        for (var [id, room] of rooms.entries()){       
            for (var peer of room.getPeers()){
                ...
                this.addConsumer(peer.data.workerPid, 1 + peer.data.producers.size)
            }
