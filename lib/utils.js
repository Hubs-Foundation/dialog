...
class workerLoadMan{
    ...
    static surveyRooms(){
        this._workerLoadMap = new Map()
        for (var room of rooms){       
            logger.info("surveyRooms -- room: %s", room)
            logger.info("surveyRooms -- room._protooRoom: %s", room._protooRoom)

            for (peer of room.getPeers()){
                this.addPeer(peer.data.workerPid, 1)
            }
            for (worker of room._inuseMediasoupWorkers){
